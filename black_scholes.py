...
def plot_option_payoff(S, K, price, option_type):
    """Generate payoff diagram at expiration"""
    stock_range = np.linspace(max(0.5 * K, K - 3 * price), K + 3 * price, 100)
    
    if option_type == "call":
        payoff = np.maximum(stock_range - K, 0) - price
    else:
        payoff = np.maximum(K - stock_range, 0) - price
        
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=stock_range, y=payoff, mode='lines', name='Profit/Loss'))
    fig.add_trace(go.Scatter(x=[S], y=[0], mode='markers', marker=dict(size=10, color='red'), name='Current Price'))
    ...
    fig.add_vline(x=breakeven, line_dash="dash", line_color="green", annotation_text="Breakeven")
...
def main():
    ...
    with col1:
        ...
        with col1b:
            ...
            st.metric("Theta (daily)", f"${results['theta']:.4f}")
            st.metric("Vega (per 1%)", f"${results['vega']:.4f}")
    ...
    with col2:
        ...
        with tab1:
            ...
            # Create the heatmap with green-red colorscale instead of Blues
